...
contract Exchange is ERC20 {
     ...
     */
    function cryptoDevTokensToEth(uint _tokensSold, uint _minEth) public {
        uint256 tokenReserve = getReserve();
        //Call the getAmountOfTokens to get the amount of eth
        // that woult be returnes to the user after the swap
        ...
        // Transfer Crypto Dev Token from user's address to the contract
        ERC20(cryptoDevTokenAddress).transferFrom(
            msg.sender,
            address(this),
            _tokensSold
        );
        // Send ethBought to the user from the contract
